...
FROM
(
  SELECT ab.num, ab.company, ab.stop, sb.name
  FROM route aa
  JOIN route ab
    ON aa.company = ab.company AND aa.num = ab.num
  JOIN stops sa
    ON aa.stop = sa.id
  JOIN stops sb
    ON ab.stop = sb.id
...
JOIN 
(
  SELECT ba.stop, bb.num, bb.company
  FROM route ba
  JOIN route bb
    ON ba.company = bb.company AND ba.num = bb.num
  JOIN stops bs
    ON bb.stop = bs.id
  WHERE bs.name = 'Lochend'
) b
ON a.stop = b.stop
